...
wdImport2 <- c("E:/Study/SCI/Soil Micro/SCI/Figures/16S/QIIME2-DAD2/FourStages-DAD2-F295+R205-Filtered1/Network/FastSpar-Genus/MP")
wdOutput <- c("E:/Study/SCI/Soil Micro/SCI/Figures/16S/QIIME2-DAD2/FourStages-DAD2-F295+R205-Filtered1/Network/FastSpar-Genus/MP/0.77_p0.01")
library(stringr)#字符串处理.string manipulation
library(tidyr)
library(dplyr)

setwd(wdImport2)
data.set.name2 <- "_FastSpar_FourStages_Filtered1_MP_0.77_p0.01"
cor <- read.delim('GenusCountMPFastSparWithoutAllZero_FourStages_Filtered1_median_correlation.tsv', row.names = 1, sep = '\t', check.names = FALSE)
pvals <- read.delim('GenusCountMPFourStages_Filtered1_pvalues.tsv', row.names = 1, sep = '\t', check.names = FALSE)
...
GenusCountMP <- as.data.frame(ASVCountMPTaxonmy %>% group_by(Genus,Domain,Phylum,Class,Order,Family)%>%
                                summarise_at(vars(MP1_1:MP4_3),funs(sum)))
GenusCountMP$Genus<-substring(text=GenusCountMP$Genus,first=4,last = 1000000L)#从第4位开始取值
GenusCountMP$Phylum<-substring(text=GenusCountMP$Phylum,first=4,last = 1000000L)
rownames(GenusCountMP) <- GenusCountMP$Genus
GenusMPNode <- GenusCountMP[node_list$Id,]
GenusMPNodeTax <- GenusMPNode[,c(2:6)]
dim(GenusMPNodeTax)
NodePhylum <- GenusMPNodeTax$Phylum
NodePhylum
#Top10 phyla
...
unique(NodeColor)
GenusMPNodeTax$color <- NodeColor
node_listTax <- cbind(node_list,GenusMPNodeTax)
setwd(wdOutput)
